defmodule BandcampScraperWeb.CoreComponents do
  @moduledoc """
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="shadow-zinc-900/50 ring-zinc-600 relative hidden rounded-2xl bg-zinc-800 p-14 shadow-lg ring-1 transition"
            >
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 rounded-lg bg-zinc-100 hover:bg-white py-2 px-3",
    ...
    <div phx-feedback-for={@name}>
      <label class="flex items-center gap-4 text-sm leading-6 text-zinc-300">
        ...
        <input
          ...
          value="true"
          checked={@checked}
          class="rounded border-zinc-600 bg-zinc-800 text-zinc-100 focus:ring-0"
          {@rest}
        />
        <%= @label %>
      </label>
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-2 block w-full rounded-md border border-zinc-600 bg-zinc-800 text-zinc-100 shadow-sm focus:border-zinc-500 focus:ring-0 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
  ...
  def input(%{type: "textarea"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <textarea
        ...
        name={@name}
        class={[
          "mt-2 block w-full rounded-lg bg-zinc-800 text-zinc-100 focus:ring-0 sm:text-sm sm:leading-6",
          "min-h-[6rem] phx-no-feedback:border-zinc-600 phx-no-feedback:focus:border-zinc-500",
          ...
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-zinc-800">
            ...
            <td :if={@action != []} class="relative w-14 p-0">
              <div class="relative whitespace-nowrap py-4 text-right text-sm font-medium">
                ...
                <span
                  :for={action <- @action}
                  class="relative ml-4 font-semibold leading-6 text-zinc-100 hover:text-white"
                >
    ...
    <div class="mt-14">
      <dl class="-my-4 divide-y divide-zinc-700">
        <div :for={item <- @item} class="flex gap-4 py-4 text-sm leading-6 sm:gap-8">
          <dt class="w-1/4 flex-none text-zinc-400"><%= item.title %></dt>
  ...
  def back(assigns) do
    ...
    <div class="mt-16">
      <.link
        navigate={@navigate}
        class="text-sm font-semibold leading-6 text-zinc-300 hover:text-white"
      >
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(BandcampScraperWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(BandcampScraperWeb.Gettext, "errors", msg, opts)
    end
  end
